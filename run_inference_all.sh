...
for i in $(seq 0 4 $(($NUM_MODELS-1))); do
  ...
  tmux_start_inference 0 ./run_inference.sh INFERENCE_TEST.record ${models[i]} 0
  tmux_start_inference 1 ./run_inference.sh INFERENCE_TEST.record ${models[i + 1]} 1
  tmux_start_inference 2 ./run_inference.sh INFERENCE_TEST.record ${models[i + 2]} 2
  tmux_start_inference 3 ./run_inference.sh INFERENCE_TEST.record ${models[i + 3]} 3
  sleep 2m
...
for i in $(seq 0 1 $(($NUM_MODELS-1))); do
  ./run_inference.sh INFERENCE_TEST.record ${models[i]} -1
done
